...
def solve_lp3_aug(
    ...
    - PCTL-related quantities (goal_prob, region_flag_prob, until_prob) are
      always *computed*. This is because with no heuristics we do not
    ...
    for c in extra_constraints:
      ...
      if c.kind == "visit_region_max":
        # Need an admissible UPPER bound on remaining probability mass.
        if partial:
            if H_region_upper is None or c.region_name not in H_region_upper:
                raise ValueError(
                    f"visit_region_max({c.region_name}) needs H_region_upper on partial envelopes "
                    f"(closest-to-Trevizan: constraints always enforced)."
                )
        ...
        # Need an admissible LOWER bound on remaining probability mass.
        if partial:
            if H_region_lower is None or c.region_name not in H_region_lower:
                raise ValueError(
                    f"visit_region_min({c.region_name}) needs H_region_lower on partial envelopes "
                    f"(closest-to-Trevizan: constraints always enforced)."
                )
        ...
        if partial:
            if H_until_lower is None or c.spec_name not in H_until_lower:
                raise ValueError(
                    f"until_min({c.spec_name}) needs H_until_lower on partial envelopes "
                    f"(closest-to-Trevizan: constraints always enforced)."
                )
        ...
        constraints.append(expr >= c.bound)
          ...
          if partial:
            if H_until_upper is None or c.spec_name not in H_until_upper:
                raise ValueError(
                    f"until_max({c.spec_name}) needs H_until_upper on partial envelopes "
                    f"(closest-to-Trevizan: constraints always enforced)."
                )
